...
def equatorial_to_horizontal(latitude, longitude, timestamp, right_ascension,
    ...
    """
    lst = clock.gps_to_lst(timestamp, longitude)
#     print "LST  is "+repr(lst) + "LST is close "
    
    ha = (angles.hours_to_radians(lst) - right_ascension)
    ha %= 2 * pi
#     print "HA In Radians is "+repr(ha) 
#     print "HA In degrees is "+repr(math.degrees(ha)) 
...
#     print '高度 in radians' + repr(altitude)
#     print '高度 in degrees' + repr(math.degrees(altitude))
    Azimuth = arccos((sdec - (slat * sin(altitude))) / (clat * cos(altitude)))
#     print '方位 in radians' + repr(Azimuth)
#     print '方位 in degrees' + repr(math.degrees(Azimuth))

    if sha > 0:
        Azimuth = 2 * pi - Azimuth
#     print '修正方位 in radians' + repr(Azimuth)
#     print '修正方位 in degrees' + repr(math.degrees(Azimuth))
    zenith, azimuth = horizontal_to_zenithazimuth(altitude, Azimuth)

    return math.degrees(altitude),math.degrees(Azimuth),zenith, azimuth
